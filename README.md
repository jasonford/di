...
- The bundled `broot` patches make watch mode refresh recursively from the current root and show git status plus `+/-` counts in a fixed gutter on changed files and directories
...
- The broot tree uses that same fixed gutter to mark open Neovim buffers with `=` for clean buffers and `*` for buffers with unsaved edits
