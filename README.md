...
- `./develop` opens a tmux session with Codex in the left pane and Yazi in the right pane
...
- In Yazi, open text/code/config files to edit them in `codex-nvim`; a dedicated `yazi-edit` wrapper forwards the selected path safely into Neovim, and when Neovim exits, Yazi resumes in the same pane.
- In Neovim:
...
- Yazi is pinned with `YAZI_VERSION=... ./develop`
- Yazi runs with an iconless theme by default so it renders cleanly in stock terminal setups. Set `YAZI_ICONS=1 ./develop` to opt back into Nerd Font icons.
- Yazi icons depend on the host terminal font, not the container image. In terminals like Ubuntu's default Terminal app, install and select a Nerd Font if you want icon glyphs to render correctly.
